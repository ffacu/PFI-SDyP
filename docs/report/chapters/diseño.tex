...
\begin{enumerate}
    \item \textbf{Conversión a Escala de Grises:} A partir de la imagen original a color, se genera una copia en escala de grises utilizando la fórmula de luminancia ponderada ITU-R BT.601 \cite{gonzalez2018digital}, ya que esta es la que mejor representa la percepción de brillo del ojo humano:
    \begin{equation}
    \label{eq:luminancia}
...
Esta etapa consistió en paralelizar el código secuencial utilizando la especificación OpenMP \cite{dagum1998openmp}, aprovechando que todos los núcleos de un mismo nodo comparten el mismo espacio de direcciones de memoria. OpenMP utiliza un modelo de ejecución \textit{fork-join}: el programa corre de manera secuencial hasta que encuentra una región paralela, momento en el cual el hilo principal genera un equipo de hilos de trabajo que ejecutan el código en simultáneo, para luego volver a unirse (\textit{join}) al finalizar dicha región.


\subsection{Minimalismo Invasivo}

Uno de los aspectos más destacables de esta etapa fue lo poco que tuvo que modificarse el código original para obtener paralelismo. Dado que el procesamiento de cada píxel es una operación independiente del resultado de los píxeles vecinos recién calculados (no existen dependencias de datos entre iteraciones), fue posible insertar directivas \texttt{\#pragma omp parallel for} directamente sobre los bucles más externos de cada función de filtrado (los que recorren la altura de la imagen), sin tener que reescribir ni una sola línea de la lógica matemática de los filtros. Esto permitió mantener una única base de código ``conceptual'' entre la versión secuencial y la paralela, reduciendo enormemente el riesgo de introducir errores nuevos durante la paralelización.
...
Al momento de elegir la política de planificación del bucle paralelo, se optó por \texttt{schedule(static)} en lugar de las alternativas dinámicas (\texttt{dynamic} o \texttt{guided}). Esta decisión se basa en la naturaleza misma del problema: el procesamiento de una imagen es un caso de carga de trabajo completamente predecible y uniforme, ya que el costo computacional de aplicar un filtro a un píxel es idéntico para cualquier píxel de la imagen (no hay píxeles ``más difíciles'' que otros). Bajo estas condiciones, la planificación estática divide el espacio total de iteraciones en bloques grandes y contiguos, asignándolos de manera fija a cada hilo desde el inicio.

Esto trae dos beneficios concretos. Primero, se elimina por completo el \textit{overhead} de tener que consultar al planificador en tiempo de ejecución para repartir nuevas tandas de trabajo, como ocurriría con una planificación dinámica, lo cual sería un costo innecesario dado que ningún hilo terminará significativamente antes que otro. Segundo, al asignarle a cada hilo un bloque grande y contiguo de filas, se respeta la localidad espacial de la memoria caché, favoreciendo los mecanismos de \textit{prefetching} del hardware. Como ventaja adicional, al estar los bloques de cada hilo separados por una gran cantidad de memoria, se evita por diseño el problema de \textit{false sharing} (donde distintos hilos escriben en variables que comparten la misma línea de caché), por lo que no fue necesario recurrir a mecanismos de sincronización costosos como \texttt{\#pragma omp critical} dentro de los bucles de procesamiento.


\subsection{Prevención de Condiciones de Carrera}

Si bien la lectura de la imagen fuente y la escritura sobre la imagen de destino se realizan sobre arreglos compartidos por todos los hilos, existen ciertas variables auxiliares que sí representan un riesgo de condición de carrera si se manejan incorrectamente. El ejemplo más claro es el cálculo del filtro de Sobel, donde para cada píxel se necesitan acumular los resultados parciales de la convolución horizontal y vertical en variables como \texttt{sumX} y \texttt{sumY}.
...
Dado que crear un equipo de hilos en OpenMP no es una operación gratuita (implica cierto \textit{overhead} de sincronización al iniciar y finalizar la región paralela), paralelizar un bucle de apenas 256~iteraciones resultaría contraproducente: el costo de administrar los hilos superaría ampliamente el tiempo que se ahorraría en el cálculo en sí, que de por sí es trivial. Por este motivo, se decidió dejar esta función ejecutándose de forma puramente secuencial, reservando el paralelismo para las operaciones que realmente representan un costo computacional significativo, como son los filtros aplicados sobre los millones de píxeles de la imagen.


%--------------------------------------------------------------------
\section{Paralelización en memoria distribuida (MPI)}

En esta etapa el foco estuvo en extender el procesamiento más allá de los límites de un solo nodo, distribuyendo la carga de trabajo entre múltiples procesos independientes mediante el estándar MPI \cite{gropp1999using}. A diferencia de OpenMP, en este modelo cada proceso (\textit{rank}) cuenta con su propio espacio de memoria privado, por lo que cualquier dato que un proceso necesite y no posea localmente debe ser explícitamente enviado y recibido a través de la red.


\subsection{Descomposición de Dominio 1D por Filas}

La primera decisión de diseño fue cómo dividir la imagen entre los distintos procesos. Se descartó una descomposición en bloques 2D (dividir la imagen en una grilla de rectángulos), ya que, si bien teóricamente minimiza la relación entre el perímetro y el área de cada subdominio (reduciendo el volumen total de datos de frontera), en la práctica requeriría transmitir columnas de píxeles, las cuales no son contiguas en memoria. Esto obligaría a empaquetar y desempaquetar constantemente datos dispersos, ya sea mediante tipos de datos derivados de MPI o mediante copias manuales, generando un \textit{overhead} de CPU que terminaría anulando cualquier beneficio de la descomposición 2D.

Por este motivo, se optó por una descomposición de dominio unidimensional por filas: cada proceso recibe un bloque contiguo de filas completas de la imagen. Como cada fila ya se encuentra almacenada de manera contigua en el arreglo unidimensional (tal como se explicó en la sección~2.1), el proceso raíz puede repartir estos bloques directamente mediante punteros de memoria, sin necesidad de ningún empaquetado adicional, logrando un reparto de altísimo rendimiento.

Sin embargo, surge un problema práctico: la altura de la imagen rara vez es perfectamente divisible por la cantidad de procesos disponibles. Para resolver esto, el proceso raíz calcula cuántas filas le corresponden a cada proceso mediante una división entera, y además calcula el \textit{remainder} (resto) de dicha división. Este resto se distribuye repartiendo una fila extra a los primeros procesos hasta agotarlo, de modo que la diferencia de carga entre el proceso que más filas recibe y el que menos recibe sea, como máximo, de una sola fila:

\begin{lstlisting}[language=C, caption={Descomposición de dominio 1D con distribución balanceada del resto}, label=list:domain_decomp]
...
Para llevar a cabo esta distribución desigual de manera eficiente, se utilizaron las funciones \texttt{MPI\_Scatterv} y \texttt{MPI\_Gatherv} en lugar de sus contrapartes \texttt{MPI\_Scatter} y \texttt{MPI\_Gather}, que únicamente permiten repartir fragmentos de igual tamaño. Estas funciones reciben como parámetro un arreglo de cantidades, donde cada posición indica cuántos elementos le corresponden a cada proceso, y un arreglo de desplazamientos, que indica desde qué posición del arreglo original comienza el bloque de cada proceso. Gracias a esto, fue posible lograr un reparto exacto y balanceado de la carga, sin desperdiciar ni un solo byte ni dejar procesos ociosos.


\subsection{Mecanismo de Halo Exchange (Zonas Fantasma)}

El mayor desafío de esta etapa surge al momento de aplicar los filtros de convolución (borroneado y Sobel) sobre los bordes de cada subdominio. El problema es el siguiente: para calcular el nuevo valor de un píxel ubicado en la primera o última fila del bloque asignado a un proceso, el filtro necesita leer los valores de las filas vecinas, que físicamente se encuentran en la memoria de otro proceso (el que maneja la partición superior o inferior). Como cada proceso sólo tiene acceso a su propia memoria local, esta información no está disponible directamente.
...
Antes de comenzar el procesamiento propiamente dicho, todos los procesos participan de una fase de comunicación destinada a llenar estas zonas fantasma con las filas reales correspondientes de sus vecinos. Cabe destacar que los procesos en los extremos del dominio (el primero y el último) se comportan de manera distinta a los procesos intermedios: mientras que un proceso intermedio tiene tanto un vecino superior como uno inferior con quienes intercambiar filas, los procesos extremos solo tienen un vecino de un lado, y del otro lado simplemente no existe información que recibir (corresponde al borde real de la imagen completa).


\subsection{Comunicación Segura y Prevención de Deadlocks}

Para implementar este intercambio de filas fantasma se diseñó una función modular, \texttt{halo\_exchange}, encargada de encapsular toda la lógica de comunicación entre vecinos. Un enfoque ingenuo para este intercambio sería utilizar pares de llamadas bloqueantes \texttt{MPI\_Send} y \texttt{MPI\_Recv} de manera secuencial; sin embargo, si todos los procesos intentan enviar datos al mismo tiempo sin que ninguno haya publicado aún un buffer de recepción, los buffers internos de la red pueden saturarse, provocando un \textit{deadlock} que detiene la ejecución del programa por completo.

Para evitar este riesgo, la función \texttt{halo\_exchange} se implementó utilizando la primitiva \texttt{MPI\_Sendrecv}, que combina en una sola llamada una operación de envío y una de recepción de forma simultánea, dejando que la propia biblioteca de MPI se encargue internamente de coordinar el intercambio sin riesgo de bloqueo cíclico. El intercambio completo se realiza en dos pasos simétricos: primero un intercambio ``ascendente'', donde cada proceso envía sus filas reales superiores a su vecino de arriba y recibe de él sus filas fantasma superiores; y luego un intercambio ``descendente'', análogo pero en sentido contrario, con el vecino de abajo.
...
Finalmente, para evitar tener que escribir lógica condicional especial (\texttt{if/else}) que distinga el comportamiento de los procesos extremos (\textit{rank}~0 y el último \textit{rank}) respecto de los procesos intermedios, se aprovechó el destino virtual \texttt{MPI\_PROC\_NULL} que ofrece MPI. Cuando un proceso extremo no tiene un vecino real hacia un lado, simplemente se configura ese ``vecino'' como \texttt{MPI\_PROC\_NULL}. Al ejecutar un \texttt{MPI\_Sendrecv} dirigido a \texttt{MPI\_PROC\_NULL}, la operación se completa de manera inmediata y exitosa sin realizar ninguna transferencia real de datos (una especie de \textit{no-op}), permitiendo que exactamente el mismo bloque de código de \texttt{halo\_exchange} funcione de manera uniforme para todos los procesos, sin distinguir casos especiales.


%--------------------------------------------------------------------
\section{Arquitectura híbrida (MPI + OpenMP)}

La última versión del proyecto combina los dos modelos de paralelismo desarrollados anteriormente, buscando aprovechar la jerarquía física de un clúster moderno: por un lado, múltiples nodos conectados por una red, y por otro lado, dentro de cada nodo, múltiples núcleos de CPU que comparten memoria. La idea central de esta arquitectura es lanzar un único proceso MPI por nodo (en lugar de uno por núcleo, como ocurriría en una versión MPI pura), y que dicho proceso utilice internamente OpenMP para repartir su porción de trabajo entre todos los núcleos disponibles en su nodo.


\subsection{Nivel de Soporte Multi-Hilo}

Al combinar MPI con multithreading dentro de un mismo proceso, surge un riesgo adicional: si varios hilos de OpenMP intentaran invocar funciones de MPI de manera simultánea, el estado interno de la biblioteca de MPI podría corromperse, provocando fallos de segmentación. Para evitar esto, MPI ofrece distintos niveles de soporte de hilos, que se solicitan explícitamente mediante la función \texttt{MPI\_Init\_thread} (en lugar del clásico \texttt{MPI\_Init}).

Para este proyecto se eligió el nivel \texttt{MPI\_THREAD\_FUNNELED}. Bajo esta política, la aplicación puede tener múltiples hilos en ejecución, pero únicamente el hilo principal (el \textit{Master Thread}, que es el que invocó originalmente a \texttt{MPI\_Init\_thread}) tiene permitido realizar llamadas a funciones de MPI. El resto de los hilos de trabajo (los hilos OpenMP \textit{Workers}) quedan restringidos a operar exclusivamente sobre la RAM local del nodo, sin ningún tipo de acceso a la red. Esto evita por completo cualquier posibilidad de congestión o corrupción en las comunicaciones de red, y a su vez evita el costo de rendimiento que implicaría solicitar un nivel de soporte más permisivo (como \texttt{MPI\_THREAD\_MULTIPLE}), el cual requeriría que la biblioteca de MPI mantenga bloqueos internos constantes para proteger su estado.


\subsection{Flujo de Trabajo Jerárquico}

La aplicación híbrida orquesta ambos modelos de paralelismo de manera secuencial dentro de cada ``ronda'' de procesamiento, alternando entre comunicación de red y cómputo local. Primero, a nivel de MPI, el proceso raíz divide la imagen completa en bloques de filas (utilizando la misma lógica de \texttt{MPI\_Scatterv} descrita en la sección~2.3) y los distribuye entre los distintos procesos, uno por nodo. Inmediatamente después, todos los procesos participan del intercambio de halos (\textit{halo exchange}) descrito anteriormente, para que cada uno tenga en su memoria local todas las filas (propias y fantasma) que va a necesitar.

Una vez que esta fase de sincronización de red finaliza y la memoria local de cada proceso queda completamente coherente, el \textit{Master Thread} de cada proceso invoca a su equipo de hilos OpenMP mediante las directivas \texttt{\#pragma omp parallel for}, ya implementadas en la versión de memoria compartida. A partir de este punto, todos los núcleos del nodo trabajan en simultáneo sobre la porción de imagen que le fue asignada a ese proceso MPI, aplicando los filtros de borroneado, Sobel y posterizado sobre sus filas correspondientes, sin necesidad de ninguna comunicación adicional con otros nodos durante esta fase de cómputo. Recién al finalizar el procesamiento, el proceso raíz recolecta los resultados de todos los nodos mediante \texttt{MPI\_Gatherv} para ensamblar la imagen final.

De esta manera, la red solo se utiliza para las operaciones estrictamente necesarias (repartir la imagen, intercambiar halos y recolectar resultados), mientras que la gran mayoría del tiempo de cómputo se realiza de forma local, aprovechando al máximo el ancho de banda de memoria compartida dentro de cada nodo.
