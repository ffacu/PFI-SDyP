...
void convert_to_grayscale(unsigned char *img_in, unsigned char *img_out, int width, int height) {
    ...
    // Iteramos sobre todos los pixeles
    for (int i = 0; i < num_pixels; i++) {
        // img_in tiene 3 canales por pixel, img_out solo 1
        int idx_rgb = i * 3;
...
void apply_blur(unsigned char *img_in, unsigned char *img_out, int width, int height, int filter_size) {
    ...
    // Llenamos toda la imagen de salida con negro (0) inicialmente, por ende
    // los bordes auxiliares neutros son negros
...
// Deteccion de bordes (Sobel + umbralado)
void apply_sobel(unsigned char *img_in, unsigned char *img_out, int width, int height, int threshold) {
    ...
    for (int y = offset; y < height - offset; y++) {
        for (int x = offset; x < width - offset; x++) {
            ...
            // Si la magnitud supera el umbral, es un borde (lo pintamos de negro)
            // Si no lo supera, es fondo (lo pintamos de blanco)
